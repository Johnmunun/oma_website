...
import { EventsGridSkeleton } from "@/components/ui/content-skeletons"
            ...
            {upcomingEvents.length === 0 ? (
              ...
              upcomingEvents.slice(0, 6).map((event, index) => (
                <AnimateOnScroll key={event.id} animation="fade-up" delay={index * 100}>
                  <div
                    className="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-2 group relative flex flex-col h-full card-scale-hover"
              ...
              pastEvents.slice(0, 8).map((event, index) => (
                <AnimateOnScroll key={event.id} animation="fade-up" delay={index * 50}>
                  <Link href={`/events/${event.slug}`}>
                    <div className="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-2 group relative h-full card-scale-hover"
                      style={{ boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)' }}
                    >
