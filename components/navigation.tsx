  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 py-4 max-w-full overflow-x-hidden">
          ...
          <Link href="/" className="flex items-center gap-2 sm:gap-3 group min-w-0 flex-1 sm:flex-initial">
            ...
            {isLoading && !logoUrl && !cachedLogo ? (
              ...
              <div className="relative inline-flex items-center justify-center flex-shrink-0 group/logo">
                {/* Fond élégant avec gradient et ombre professionnelle */}
                <div className="absolute w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-white via-white to-gray-50 rounded-2xl shadow-lg shadow-black/10 border border-white/50 z-0 transition-all duration-300 group-hover/logo:shadow-xl group-hover/logo:shadow-gold/20 group-hover/logo:scale-105" />
                {/* Effet de glow doré au hover */}
                <div className="absolute w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-gold/20 via-gold/10 to-transparent rounded-2xl z-[1] opacity-0 group-hover/logo:opacity-100 transition-opacity duration-300 blur-sm" />
                {/* Logo par-dessus - responsive avec meilleure qualité */}
                <div className="relative z-10 p-1.5 sm:p-2 transition-transform duration-300 group-hover/logo:scale-105">
                  <Image
                    src={logoUrl}
                    alt={siteTitle}
                    width={56}
                    height={56}
                    className="h-11 w-auto sm:h-14 object-contain drop-shadow-sm"
        ...
        {/* Mobile Navigation */}
        {isMobileMenuOpen && (
          <div id="mobile-navigation" className="md:hidden mt-4 pb-4 space-y-4" role="menu">
            {navLinks.map((link) => {
              if (link.isHome) {
                return (
                  <Link
