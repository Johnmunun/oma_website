...
export default function AdminMessages() {
  ...
  const toggleHide = async (messageId: string, isHidden: boolean) => {
    try {
      const res = await fetch(`/api/admin/messages?id=${messageId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json()
      if (data.success) {
        toast.success(`Message ${!isHidden ? 'masqué' : 'affiché'} avec succès`)
        loadMessages()
        if (selectedMessage?.id === messageId) {
          setSelectedMessage(data.data)
        }
      ...
      console.error("[Admin] Erreur masquage message:", err)
      toast.error("Erreur lors de la mise à jour")
    }
  }
              ...
              <div className="space-y-4">
                ...
                {canEdit && (
                  <div className="space-y-2 pt-4 border-t border-border">
                    ...
                    <Button
                      variant={selectedMessage.isHidden ? "outline" : "secondary"}
                      size="sm"
                      onClick={() => toggleHide(selectedMessage.id, selectedMessage.isHidden)}
                      className="w-full"
                    >
