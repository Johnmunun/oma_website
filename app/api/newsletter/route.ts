...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitResultIP = await checkRateLimit(ip, RATE_LIMIT_CONFIGS.newsletter)
    
    if (!rateLimitResultIP.allowed) {
      const resetIn = Math.ceil((rateLimitResultIP.resetAt.getTime() - Date.now()) / 1000 / 60)
      return NextResponse.json(
        ...
        { 
          status: 429,
          headers: {
            'X-RateLimit-Limit': RATE_LIMIT_CONFIGS.newsletter.maxRequests.toString(),
            'X-RateLimit-Remaining': rateLimitResultIP.remaining.toString(),
            'X-RateLimit-Reset': rateLimitResultIP.resetAt.toISOString(),
          },
        }
      )
    }
    ...
    // Rate limiting supplémentaire par email
    const email = validatedData.email.trim().toLowerCase()
    const rateLimitResultEmail = await checkRateLimit(
      email, 
      { ...RATE_LIMIT_CONFIGS.newsletter, keyPrefix: 'newsletter-email' }
    )
    
    if (!rateLimitResultEmail.allowed) {
      const resetIn = Math.ceil((rateLimitResultEmail.resetAt.getTime() - Date.now()) / 1000 / 60)
      return NextResponse.json(
        { 
          success: false, 
          error: `Trop de tentatives pour cet email. Veuillez réessayer dans ${resetIn} minute(s).`,
        },
        { status: 429 }
      )
    }
