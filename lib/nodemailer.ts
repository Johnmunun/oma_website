...
async function getSMTPConfig() {
  ...
  try {
    ...
    // Support pour Hostinger et autres fournisseurs
    ...
    const config = {
      host: settingsMap.smtp_host || defaultHost,
      port: parseInt(settingsMap.smtp_port || process.env.SMTP_PORT || String(defaultPort)),
      secure: settingsMap.smtp_secure === 'true' || (settingsMap.smtp_secure === undefined ? defaultSecure : settingsMap.smtp_secure === 'true'),
...
async function createTransporter() {
  ...
  try {
    const transporter = nodemailer.createTransport({
      ...
      // Options supplémentaires pour compatibilité (Gmail, Hostinger, etc.)
      tls: {
        rejectUnauthorized: process.env.NODE_ENV === 'production', // Rejeter les certificats invalides en production
      },
